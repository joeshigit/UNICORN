...
同一張表的每一筆資料**形狀完全相同**。每個問到的問題，每一筆都有那個欄位；空白是一個值，不是欄位消失。

| 型別 | 空白時 | 怎麼查空白 |
|------|--------|-----------|
| 文字／數字／日期／時間 | `null` | `where(key, '==', null)` |
| 下拉 | `[]` / `''` / `0` | `where(keyCount, '==', 0)` |
| 檔案 | `0` | `where(key, '==', 0)` |

若讓空白的欄位直接消失，那個欄位在資料庫裡就是索引上的一個洞：任何條件都撈不到（連 `!=` 也不行），依它排序時**整筆紀錄會消失**，也查不到誰沒填。`null` 是真正的值，所以這些問題都不存在。
